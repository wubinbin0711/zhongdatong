...
- 角色权限
  - 超级管理员（`PLATFORM_ADMIN`）：可新增企业管理账号/子账号，可禁用其登录
  - 企业管理账号（`ADMIN`）：仅可新增和管理自己名下子账号，可新建/删除订单
  - 企业子账号（`SUB_ACCOUNT`）：仅可查看“其上级企业管理账号创建”的订单，可改状态，不可删除
  - 企业管理账号命名规则：账号后缀必须是 `01`、`02` 或 `03`
- 订单字段
  - `content`（整段文本）
  - `ownerCode`（负责人编号）
  - `status`（可随意切换）
